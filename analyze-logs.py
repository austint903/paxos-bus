...
paxosbus/logs/gcp/, and paxosbus/logs/cloudlab/, where run-paxosbus.sh, run-gcp.sh
and run-cloudlab.sh archive one directory per run.
...
def main():
    if len(sys.argv) > 1:
        ...
        runs = (glob.glob("paxosbus/logs/local/local-run-*") +
                ...
                glob.glob("paxosbus/logs/cloudlab/cloudlab-run-*"))
