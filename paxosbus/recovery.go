...
func (r *Replica) driveViewChange(vc *vcState) {
	...
	installMax, hasInstallMax := plan.maxSlot, plan.hasMax
	if plan.hasStable && (!hasInstallMax || installMax < plan.stableSlot) {
		// A report bitmap describes only the suffix above stableSlot, so a
		// committed-only merge may not otherwise carry a MaxSlot. The freshly
		// fetched prefix still has to execute behind the ViewChange fence before
		// setStableLocked can publish the selected commit point.
		installMax, hasInstallMax = plan.stableSlot, true
	}
	r.installViewLocked(vc.view, plan.stableSlot, plan.hasStable,
		installMax, hasInstallMax, plan.noops, false)
	r.vc = nil
	r.mu.Unlock()
